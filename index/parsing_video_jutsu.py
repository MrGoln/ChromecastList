...
redact_serias = 96 # выбрать с какой серии начать запись в csv
with open('videos_new.csv', 'w') as file:
    file.writelines(lines[:redact_serias - 1])

url_videos = []
for i in range(redact_serias, 153):
    driver.get(f'https://jut.su/dragonball/season-1/episode-{i}.html')
    with open('videos_new.csv', 'a', newline='') as csvfile:
        spamwriter = csv.writer(csvfile, delimiter=' ',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
        spamwriter.writerow([driver.find_element(By.XPATH, '/html/body/div[5]/div[1]/div[2]/div/div[4]/div[1]/div[1]/div[2]/div[2]/video/source[1]').get_attribute('src')])
